...
{							//want to read, then move to next entry.
	...
	if ((curr_ent) >= (Q_ptr -> circular_buffer[o].entrynum) && ((curr_ent) <= (Q_ptr -> circular_buffer[i-1].entrynum))) {
		...
		t-> message = Q_ptr -> circular_buffer[want_ind-1].message; //grabs new message, sets
...
int main(int argc, char *argsv[])
{
   ...
   for(i = 0; i < NUMTOPICS; i++)
    ...
    {
        error = pthread_create(&publisher[i], NULL, &pub, NULL);
        if (error != 0)
    ...
    {
        error = pthread_create(&subscriber[i], NULL, &sub, NULL);
        if (error != 0)
            printf("\n Thread can't be created : [%s]\n", strerror(error));
    }

    pthread_t delete[num_dels];
    for(i = 0; i < num_dels; i++)
    {
        error = pthread_create(&delete[i], NULL, &del, NULL);
        if (error != 0)
            printf("\n Thread can't be created : [%s]\n", strerror(error));
    }
    ...
    return 0;
    printf("out pointer: %d in: %d\n", Topic_Q_ptr.out, Topic_Q_ptr.in);
}
